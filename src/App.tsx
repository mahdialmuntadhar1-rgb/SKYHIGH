...
export default function App() {
  ...
  const [selectedCity, setSelectedCity] = useState(IRAQ_CITIES[0].name);
  const [selectedDistrict, setSelectedDistrict] = useState(IRAQ_CITIES[0].districts[0].name);
  ...
  const startCollection = async () => {
    if (selectedSources.length === 0) {
      setRunError('Please select at least one active source.');
      return;
    }
    ...
    try {
      ...
      setRunResult(payload as DiscoveryResult);
      setDataVersion((version) => version + 1);
    } catch (error: any) {
      setRunError(error.message || 'An unexpected server error occurred.');
    } finally {
      setIsRunning(false);
    }
  };
        ...
        return (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-8 h-full flex flex-col">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 flex-1 min-h-0">
              <SourceSelector selectedIds={selectedSources} onChange={setSelectedSources} />
              <JobSetup
                ...
                runResult={runResult}
                runError={runError}
                onCityChange={(city, district, zone) => {
                  setSelectedCity(city);
                  setSelectedDistrict(district);
                  setSelectedZone(zone);
                }}
                onDistrictChange={(district, zone) => {
                  setSelectedDistrict(district);
                  setSelectedZone(zone);
                }}
      ...
      case 'review':
        return (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
            <ReviewQueue dataVersion={dataVersion} />
          </motion.div>
        );
      case 'upload':
        return (
