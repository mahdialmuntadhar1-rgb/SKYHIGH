...
import { Bell, Search, ChevronRight, BarChart3 } from 'lucide-react';
...
export default function App() {
  ...
  const [lastRunResult, setLastRunResult] = useState<DiscoveryResult | null>(null);

  const handleCollectionSuccess = (result: DiscoveryResult) => {
        ...
        return (
          ...
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 flex-1 min-h-0">
              <SourceSelector selectedIds={selectedSources} onSelectionChange={setSelectedSources} />
              <JobSetup
                selectedSources={selectedSources as DiscoverySource[]}
                onRunSuccess={handleCollectionSuccess}
              />
            </div>
            {lastRunResult && (
              <div className="text-xs text-zinc-500">
                Last run: inserted {lastRunResult.insertedCount}, skipped {lastRunResult.skippedCount}
              </div>
            )}
          </motion.div>
        );
        ...
        return (
          <div className="flex flex-col items-center justify-center h-[60vh] text-center">
            ...
            <p className="text-zinc-500 text-sm max-w-xs">
              The {activeTab} module is currently being optimized for Iraq city-center data structures.
            </p>
          </div>
        );
    }
