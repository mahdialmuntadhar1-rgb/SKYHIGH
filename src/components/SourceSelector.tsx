...
export function SourceSelector({ selectedSources, onChange }: SourceSelectorProps) {
  ...
  const activeSources = useMemo(() => SOURCES.filter((s) => s.enabled), []);

  const toggleAll = () => {
    if (selectedSources.length === activeSources.length) {
      onChange([]);
    } else {
      onChange(activeSources.map((s) => s.id));
    }
  };
    ...
    <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm overflow-hidden flex flex-col h-full">
      <div className="p-6 border-b border-zinc-100 bg-zinc-50 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-zinc-500 font-medium uppercase tracking-wider">Select APIs for collection</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            {selectedSources.length === activeSources.length ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />}
            {selectedSources.length === activeSources.length ? 'Deselect All' : 'Select All'}
          </button>
        </div>
      </div>
              ...
              >
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {!source.enabled ? (
                        <span className="text-[9px] font-black text-zinc-500 uppercase tracking-widest">Not in active flow</span>
                      ) : source.freeTier ? (
                        <span className="text-[9px] font-black text-emerald-600 uppercase tracking-widest">Free Tier Available</span>
                      ) : (
                        <span className="text-[9px] font-black text-amber-600 uppercase tracking-widest">API Key Required</span>
                      )}
                    </div>
                  </div>
                </td>
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    {source.bulkSupport && <div className="p-1.5 bg-zinc-100 rounded-lg text-zinc-500"><Layers className="w-3.5 h-3.5" /></div>}
                    {source.socialSupport && <div className="p-1.5 bg-zinc-100 rounded-lg text-zinc-500"><Users className="w-3.5 h-3.5" /></div>}
                    <div className="p-1.5 bg-zinc-100 rounded-lg text-zinc-500"><Zap className="w-3.5 h-3.5" /></div>
                  </div>
                </td>
                <td className="px-6 py-4 text-center">
      ...
      <div className="p-4 bg-zinc-50 border-t border-zinc-100 flex items-center gap-3">
        ...
        <p className="text-xs text-zinc-500 font-medium">
          <span className="font-bold text-zinc-900">{selectedSources.length} sources selected.</span> Only active sources can run in production flow.
        </p>
      </div>
    </div>
  );
