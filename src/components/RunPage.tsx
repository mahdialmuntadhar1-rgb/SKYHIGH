...
const SOURCES: { id: DiscoverySource; name: string }[] = [
  ...
  { id: 'gemini', name: 'Gemini Research (LLM-assisted)' }
];
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="bg-white border border-zinc-200 rounded-2xl p-8 shadow-sm">
        <h1 className="text-2xl font-bold mb-6">New Discovery Run</h1>

        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-wider text-zinc-500">City</label>
              <select value={city} onChange={(e) => setCity(e.target.value)} className="w-full bg-zinc-50 border border-zinc-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500/20">
                {CITIES.map((c) => (
            ...
            <div className="space-y-2">
              ...
              <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full bg-zinc-50 border border-zinc-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500/20">
                {CATEGORIES.map((c) => (
                  <option key={c} value={c}>
                    {c}
                  </option>
          ...
          <div className="mt-6 p-6 bg-zinc-900 text-white rounded-2xl">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-zinc-800 p-3 rounded-lg">
                <div className="text-xs text-zinc-500 uppercase font-bold mb-1">Inserted</div>
                <div className="text-2xl font-mono text-green-400">{result.insertedCount}</div>
              </div>
              <div className="bg-zinc-800 p-3 rounded-lg">
                <div className="text-xs text-zinc-500 uppercase font-bold mb-1">Skipped</div>
                <div className="text-2xl font-mono text-zinc-400">{result.skippedCount}</div>
              </div>
            </div>
