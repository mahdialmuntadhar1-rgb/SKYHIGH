...
function getRecentGitCommits(count = 10) {
  try {
    const output = execSync(`git log --oneline -n ${count}`, { encoding: 'utf8', stdio: 'pipe' });
...
function analyze() {
  ...
  if (recentCommits.length > 0) {
    report.info.push('## Recent Git Activity');
    recentCommits.forEach(commit => report.info.push(`- ${commit}`));
  }

  const timestamp = new Date().toISOString();
  let markdown = '# Codex Repository Status Report\n\n';
  markdown += `Generated at: ${timestamp}\n\n`;
  markdown += '## 1) Current Status\n\n';
  markdown += `- ✅ Pass checks: ${report.info.length}\n`;
  markdown += `- ⚠️ Warnings: ${report.warnings.length}\n`;
  markdown += `- ❌ Errors: ${report.errors.length}\n`;
  markdown += `- 📝 Remaining tasks: ${report.todo.length}\n\n`;
  markdown += '### Passes\n';
  markdown += `${report.info.map((line) => `- ${line}`).join('\n')}\n\n`;

  if (report.errors.length) {
    markdown += '## 2) Diagnosis\n\n';
    markdown += '### Errors (Must Fix)\n';
    markdown += `${report.errors.map((error) => `- ❌ ${error}`).join('\n')}\n\n`;
  }

  if (report.warnings.length) {
    markdown += '### Warnings (Should Address)\n';
    markdown += `${report.warnings.map((warning) => `- ⚠️ ${warning}`).join('\n')}\n\n`;
  }

  if (report.todo.length) {
    markdown += '## 3) What Is Left To Do\n\n';
    markdown += `${report.todo.map((todo) => `- [ ] ${todo}`).join('\n')}\n\n`;
  }

  markdown += '## 4) Handoff Note for Winser/Replit\n\n';
  markdown += '- Use this report to prioritize fixes in this order: **Critical Issues → Warnings → Remaining Tasks**.\n';
  markdown += '- After each milestone, run `npm run analyze:repo` to regenerate this report and track progress.\n';
  markdown += '- Re-run a full smoke test (`npm run dev`, verify core flows) before deployment.\n';

  fs.writeFileSync('CODEX_STATUS_REPORT.md', markdown, 'utf8');
  console.log('✅ Report saved to CODEX_STATUS_REPORT.md');
}
